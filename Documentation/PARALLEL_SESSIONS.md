...
| `package.json`, `CHANGELOG.md` | **C** (review) | 2026-08-20 | Cắt `4.1.0-pre.10` cho nhánh `fix/review-4.1.0-pre.9` (3 commit: af10ee4, 950107f, 17a2d82). **✅ ĐÃ TRẢ** — release commit `423b4ad`, tag `4.1.0-pre.10` = `138fcae` đã verify trên origin, nhánh đã push. Gate PASS, EditMode 165/165. |
...
| 08-19 | C | **Vòng 2 — quét drift API 2.3.1→2.9.1 + chốt backlog** (50 mục, 11 sống sót đối kháng). Quan trọng nhất cho cả hai session: (a) **`CheckForCatalogUpdates` trước 2.9 LUÔN trả rỗng** — `CanUpdateContent` đòi `Dependencies.Count == 2` trong khi build phát ra 3, nên toàn bộ đường `ApplyUpdateAsync → UpdateCatalogs → InvalidateLoaderCaches` là **code chết** trên 2.3.1 và giờ mới thật sự chạy. Cần integration test thật, đừng tin là nó đã từng chạy đúng. (b) `AssetReference.LoadAssetAsync()` là API **một lần dùng**; load lần hai trả handle invalid → `.Task` ném → caller nhận `null`. Đã đổi sang `Addressables.LoadAssetAsync<T>(RuntimeKey)`. (c) **Chưa sửa, cần người quyết:** load đang bay khi catalog update xong vẫn cache handle tiền-update (cần epoch trong `AssetLoader`); `Simple.Load` trả asset thô có thể bị invalidation huỷ dưới chân caller. |
| 08-23 | C | **Editor UI hợp nhất — `Editor/Windows/Hub/`.** Một cửa sổ `AddressableManagerHub` thay cho 4 cửa sổ / 10 tab / 21 menu item. Đáng chú ý cho session khác: (1) `IHubSection` + `CdnTabSection` — 6 tab CDN cũ chạy nguyên trong hub qua adapter, `CdnManagerWindow` **vẫn sống**, không phải sửa tab nào. (2) `SectionHealth` có **bốn** trạng thái; `NotMeasured` bắt buộc kèm lý do ở mức kiểu, và USS vẽ nó rỗng ruột — đừng thêm nhánh nào biến nó thành `Ok`. (3) `LayoutRuleProcessor` có **dry run**: cờ `_dryRun` gác 7 chỗ ghi; ai thêm chỗ ghi mới phải gác nó, `LayoutRuleDryRunTests` sẽ đỏ nếu quên. (4) `AssetLoader` thêm 2 API public đọc-chỉ: `SnapshotLoadedAssets()` và `CachedAssetCount`. (5) Menu: gỡ 3 đăng ký trùng (`Layout Rule Editor`, `Layout Viewer`, `Dashboard`) — mỗi path chỉ đăng ký **một lần**, ở class sở hữu thứ nó mở; `%&a` giờ thuộc hub, Dashboard bỏ shortcut. |
| 08-19 | C | **Vá 7 lỗ hổng review** (`Documentation/REVIEW_4.1.0-pre.9.html`). Đáng chú ý cho cả hai session: (1) `AssetLoader` giờ khoá cache `AssetReference` bằng `RuntimeKey` chứ không `AssetGUID` — sub-object sprite trước đây đè lên nhau; `AssetCacheKey.MatchesAddress` giữ cho invalidate-theo-guid vẫn với tới chúng. (2) `LayoutRuleProcessor` ghi label qua `entry.SetLabel(..., postEvent:false)` + một `BatchModification` ở cuối — `entry.labels` là **bản sao** trên Addressables 2.9+, mọi lệnh ghi cũ là no-op. (3) `AddressRule` thêm `_targetGroupTemplate` (+ DTO `RuleSerializer`) và chuẩn hoá `/`→`-` trước `FindGroup` — tên group có `/` trước đây tạo một group **mỗi asset**. (4) `CatalogVerifier`/`CatalogInspection` giờ fail khi không kiểm được gì. (5) Điểm cắm rule mới: `[SettingsRuleProvider]` + `TypeCache` (không dùng static event — mất khi domain reload = false-green). |
| 08-19 | C | **`Library/PackageCache` của `main` bị lệch manifest.** Manifest pin `com.unity.addressables 2.9.1` nhưng cache còn kẹt `@5d870284b3d9` (2.3.1) từ trước merge, nên `Tools/CompileGate/run.sh` build Addressables 2.3.1 bằng Unity 6000.5 → 455 lỗi trong dependency, gate FAIL trên code sạch. Đã đồng bộ: 2.3.1 chuyển ra `Library/_stale-addressables-2.3.1` (xoá được), copy `@8460f1c9c927` (2.9.1) vào. `Library/` là cache gitignored. **Ai gặp gate FAIL hàng loạt CS0234 thì kiểm chỗ này trước.** |
